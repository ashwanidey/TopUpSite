...
export const oneGateway = async (req, res) => {
  try {
    ...
    const response = await fetch(`https://paygapi.onegateway.in/payment/initiate`, {
      ...
      body: JSON.stringify({
        ...
        redirectUrl: `${process.env.REDIRECT_DOMAIN}/confirmation?client_txn_id=${uniqueId}`,
      }),
    });
...
export const ogOrderStatus = async (req, res) => {
  try {
